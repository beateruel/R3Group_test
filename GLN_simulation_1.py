...
class SupplyChain(object):
    def __init__(self, env, num_shopfloor,num_logistic):   
        ...
        self.labour_cost = 17 #average cost/hour in 2023 in Portugal [source = https://ec.europa.eu/eurostat/statistics-explained/index.php?title=Hourly_labour_costs]
        self.ordering_cost = 100 #€ total cost per order, assumption
        self.holding_rate = 0.2 #percentage of the products price
        self.product_price = 0.1 #€ piece
        self.client_transport_time = 24 #hours, we consider one day for delivery, .
        self.client_storage_time = 24   #hours, the order is delivered the next day than produced. 
        self.client_distribution_cost = 100 #transport cost each week 
        self.client_distribution_emissions = 100 #transport emissions each week
        
        
        
    
    def inventory_request(self, env, MTS_order, product_BOM,df_weekly_demand,week,product_name):
    ...
    def inventory_monitoring_Periodic_min_level(self, env,MTS_order,df_products_material,df_monthly_demand,PBT_price, week,init_week, end_week):  
        ...
        if (week % MTS_order.reorder_point==0):
          #  print(f"comprar material semana {week}")
            ...
            MTS_order.purchase(df_products_material,df_monthly_demand, PBT_price, int(week/4)+3,self.ordering_cost)
    ...
    def production(self, env,MTS_order, product_BOM, df_weekly_demand, week, product_name, batch_size):  
        index=1
        ...
        if(number_of_batches >0):
           # print(f'prouct name {product_name} demand {demand}, number of batches {number_of_batches}')
            ...
            if(MTS_order.stockouts[week]==0):           
                self.production_costs+=production_time_process.iloc[0]*self.labour_cost
              ...
              #  print( MTS_order.inventory_FI)
                ...
                MTS_order.total_holding_cost +=self.product_price*demand/2
    ...
    def distribution(self, env,MTS_order,df_weekly_demand, week, product_name1, product_name2):

                     
        distribution_time=self.client_transport_time+self.client_storage_time        
        ...
        if(MTS_order.stockouts[week]==0):
            ...
            self.distribution_costs += self.client_distribution_cost
            self.distribution_emissions += self.client_distribution_emissions 
            
            if(((MTS_order.inventory_FI[0,week]+MTS_order.inventory_FI[1,week])-(demand1+demand2))>0):              
                self.total_income +=(demand1*self.product_price+demand2*self.product_price)
                ...
                self.total_income +=(MTS_order.inventory_FI[0,week]*self.product_price + MTS_order.inventory_FI[1,week]*self.product_price)
...
def simulation_PI(supply_chain,MTS_order, simulation_name, variation,init_shortage,end_shortage):
    ...
    average_production_capacity_utilization =round(np.mean(100*MTS_order.production_time/ (24*7-(supply_chain.client_transport_time+ supply_chain.client_storage_time))),0)
    average_production_downtime = round(np.mean(100*MTS_order.production_downtime/ (24*7-(supply_chain.client_transport_time+ supply_chain.client_storage_time))),0)
...
def run_production_order(env,MTS_order,df_products_material, supply_chain,df_weekly_demand,week, product_name,batch_size):     
    ...
    with supply_chain.shopfloor.request() as request:#the order generates a request for a shopfloor worker to reserve  material 
        yield request        
        yield env.process(supply_chain.inventory_request(env,MTS_order,product_BOM,df_weekly_demand, week, product_name))
    ...
    with supply_chain.shopfloor.request() as request:#the order generates a request for a shopfloor worker to produce the product 1
        yield request        
        end = yield env.process(supply_chain.production(env,MTS_order,product_BOM,df_weekly_demand, week, product_name,batch_size))   
    supply_chain.weekly_order_production_time[week]+=end-init
    
    return env.now


def run_distribution_order(env,MTS_order, supply_chain,df_weekly_demand,week,product_name1,product_name2): 
    ...
    with supply_chain.logistic.request() as request:#the order generates a request for a shopfloor worker to produce the product 1
        yield request        
        yield env.process(supply_chain.distribution(env,MTS_order,df_weekly_demand, week,product_name1,product_name2))
    
    return env.now
         

def run_inventory_control(env,MTS_order,df_products_material, supply_chain,df_monthly_demand,df_weekly_demand, PBT_price, week,inventory_policy,init_week, end_week):
        ...
        a=1          
         ...
         #sourcing material: periodic review up to level
            with supply_chain.logistic.request() as request:#the order generates a request for a logistic worker to reserve  material 
                yield request                         
                yield env.process(supply_chain.inventory_monitoring_Periodic_min_level(env,MTS_order,df_products_material,df_monthly_demand, PBT_price, week,init_week, end_week))
   
def run_supplychain(env,MTS_order,df_products_material,supply_chain,df_monthly_demand,df_weekly_demand,PBT_price,init_week, end_week,policy, batch_size):
    ...
    while week < WeeksYear:
      ...
      #  print(f"inicio bucle {init/(24*7)}")
        ...
        env.process(run_inventory_control(env,MTS_order,df_products_material, supply_chain,df_monthly_demand,df_weekly_demand, PBT_price, week,policy,init_week, end_week)) 
        #yield env.timeout(1)
        init_p1 = env.now
        product_name1='E3'
        end_p1=yield env.process(run_production_order(env,MTS_order,df_products_material, supply_chain,df_weekly_demand, week,product_name1,batch_size)) 
                      
        product_name2='C3'
        yield env.process(run_production_order(env,MTS_order,df_products_material, supply_chain,df_weekly_demand, week,product_name2,batch_size))
       

        end_p2 = yield env.process(run_distribution_order(env,MTS_order,  supply_chain,df_weekly_demand,week,product_name1,product_name2))
...
def main():
    ...
    batch_size = df_main_parameters["BatchSize"].iloc[0]
    reorder_quantity =df_main_parameters["ReorderQuantity"].iloc[0]
    ...
    df_weekly_demand=gl.read_weekly_demand(demand_variation)
  
    PBT_price=gl.read_predicted_supplier_material_prices()    
    MTS_order.initialization(df_supplier_material,df_products_material,reorder_quantity)   
    ...
    MTS_order.purchase(df_products_material,df_monthly_demand, PBT_price, 0, supply_chain.ordering_cost)
    env.process(run_supplychain(env,MTS_order,df_products_material,supply_chain,df_monthly_demand,df_weekly_demand,PBT_price, init_week_shortage, end_week_shortage,policy,batch_size))
    ...
    simulation_results_PI=simulation_PI(supply_chain,MTS_order,simulation_name,demand_variation, init_week_shortage, end_week_shortage)
    return simulation_results_PI
